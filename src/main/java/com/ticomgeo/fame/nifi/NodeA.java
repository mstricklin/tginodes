...
public class NodeA extends AbstractProcessor {
    @Override
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        if (ff.getAttribute("fame.provider").equalsIgnoreCase("NodeD")) {
            FlowFile newFF = session.clone(ff);
            session.remove(ff);
            newFF = session.putAllAttributes(newFF, staticAttributes);

            session.transfer(newFF, APP_GEO_QUERY);

        } else if (ff.getAttribute("fame.provider").equalsIgnoreCase("NodeC")) {

            FlowFile newFF = session.clone(ff);
            session.remove(ff);
            newFF = session.putAllAttributes(newFF, staticAttributes);

            session.transfer(newFF, GEOLOCATION_RESPONSE);
        }
        session.commit();
